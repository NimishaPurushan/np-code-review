...
- **Scope Mismatch**: If you notice the changed files don't match what the description claims, flag it as a WARNING"""
    ...
    def _load_root_gitignore_spec() :
        try:
            gitignore_content = github.get_file_content(
                installation_id, repo_full_name, ".gitignore", ref=commit_sha
            )
            if gitignore_content:
                return parse_gitignore(gitignore_content)
        except Exception as e:
            logger.warning(f"Could not load root .gitignore for {repo_full_name}: {e}")
        return None
